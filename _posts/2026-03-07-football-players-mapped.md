...
published: true
image: /assets/images/football-players-mapped/post-image4.png
---
...
The overall plan here was to see if I could create some visualisations that used variables describing football players' performance to cluster them into groups based on the positions they each play e.g. defender, attacker, midfielder.

![](/assets/images/football-players-mapped/post-header-image3.png){:class="img"}  <update this>

### Data Sources & Scraping

The data itself was sourced from [FBref](https://fbref.com/) and scraped using a script written in Python. Across various football leagues (e.g. Premier League, La Liga), FBref holds detailed historic season-long statistics for individual players. These stats are broken down into a number of categories including:

- Standard stats e.g. minutes played, position, team, goals, assists, xG, xA
- Defence e.g. tackles, interceptions, blocks, clearances
- Passing e.g. passes attempted & successful, key passes, progressive passes
- Pass Types e.g. corners taken, free kicks taken, live passes
- Possession e.g. touches, recoveries, progressive carries, dribbles
- Shooting e.g. shots on target, shots, non-penalty xG
etc. etc.
...
After downloading the data, I combined this into one table and tidied it up, ready for running the dimensionality reduction & clustering models.

**The data was cleaned and processed using the following steps:**

Firstly, players with a low playtime were filtered out (those with less than 10 games played across the season - so about 30% ish of total potential playtime) - often players with low playtime may have better than expected stats per 90 as they come on as substitutes vs tired players and overperform compared to if they were a regular starter. 
...
Goalkeepers were also removed from the analysis. They behave very differently than outfield players, so most of the variables that describe their performance are not relevant to outfield players and vice-versa. I thus decided to focus only on outfield players here.
...
Correlation matrices were used to identify highly correlated variables and filter down the number of variables accordingly. Figure 1 below shows the final variables and the correlation between these. Some highly correlated variables still remained in the final dataset, but they tended to be measuring different aspects of performance, so I opted to leave them in. Also this is quite a pretty chart!
...
Variables were then divided into 3 categories:
...
2. Rate columns (used in modelling but no need to normalise per 90 minute match e.g. share of tackles in defensive 3rd or % of shots on target)
3. Volume columns (used in modelling, normalised per 90 to improve comparability per player e.g. goals, passes attempted or interceptions)

Finally, variables were scaled using Scikit Learn's StandardScaler function, before they could be fed into the dimensionality reduction model. This is due to both dimensionality reduction and clustering models generally being impacted by the scale of variables e.g. variables with larger numeric values may be prioritised by the model over those with smaller values. So scaling each variable to have the same mean and standard deviation can alleviate this issue and give each variable equal importance in the model.
...
The UMAP hyper-parameters were tuned through trial and error, alongside the hyper-parameter tuning of the clustering model of choice.

Regarding metrics (a hyperparameter that controls how distance is computed between points), angular/correlation metrics were the most effective at dividing players effectively between positions. The correlation metric appeared to be the most useful metric - although the cosine metric wasn't far behind.

Alongside this, a very low min_dist combined with an average n_neighbors was the most effective at generating outputs that could be usefully clustered. The low min_dist tended to clump points together more and the average n_neighbors provided a good balance in retaining global & local relationships in the model.
...
It is interesting how the models are able to easily identify and separate defenders from the rest of the outfield players - and also differentiate effectively between central defenders & wide defenders (full backs). On the other hand, the models struggle to completely differentiate between the midfielders & forwards. In a way this makes a lot of sense, as in the case of central defenders and full backs, the models are saying the points in these clusters are very similar to one another and rather different from the rest of the dataset. Likely this is because we have enough useful variables to easily split these clusters from the overall mass of players. However, for midfielders and forwards; there are perhaps not enough useful variables to split these distinctly. With the existing variables, there is probably an identifiable spectrum of offensive & defensive actions that midfielders and forwards sit on, resulting in the overlap between these clusters.

To showcase the differences between clusters and the variation within clusters in a bit more detail, we can visualise different variables across the players. This is shown below in Figure 3 for selected variables.
...
To conclude, this analysis has demonstrated how to cluster football players into positional clusters based on performance and positional data. Further improvements could be made in terms of gathering more detailed data (especially positional data) and also through enhanced feature engineering. 

Additionally, identifying different roles of players within each position (e.g. different types of strikers or central defenders) could be interesting to look into. I did get somewhere with this but it felt like I needed more/better variables and to brush up on my footballing knowledge to better understand the nuances of these different roles within positions.